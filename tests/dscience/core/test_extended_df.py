...
class TestCore:
	...
	def test_pretty(self):
		assert PrettyFrame()._repr_html_().startswith('<strong>PrettyFrame: 0 rows × 0 columns</strong>')
	...
	def test_organizingframe_read_write_csv_singleindex(self):
		path = tmpfile()
		df = SingleIndexOrg.convert(SingleIndexOrg([
			pd.Series({'abc': 1, '123': 2, 'xyz': 3}),
			pd.Series({'abc': 4, '123': 5, 'xyz': 6})
		]))
		df.to_csv(path)
	...
	def test_organizingframe_read_write_csv_multiindex(self):
		path = tmpfile()
		df = MultiIndexOrg.convert(MultiIndexOrg([
			pd.Series({'abc': 1, '123': 2, 'xyz': 3}),
			pd.Series({'abc': 4, '123': 5, 'xyz': 6})
		]))
		df.to_csv(path)
