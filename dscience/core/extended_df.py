...
class PrettyFrame(_InternalDataFrame, metaclass=abcd.ABCMeta):
	"""
...
class BaseFrame(PrettyFrame, metaclass=abcd.ABCMeta):
	"""
	...
	def sort_natural(self, column: str, alg: int = ns.INT):
		...
		df = df.sort_values('__sort').drop_cols(['__sort'])#.drop_cols(['__sort', 'level_0', 'index'])
	...
	def sort_values(self, by, axis=0, ascending=True, inplace=False,  kind='quicksort', na_position='last', **kwargs):
		return self._check_and_change(super().sort_values(by=by, axis=axis, ascending=ascending, inplace=inplace, kind=kind, na_position=na_position, **kwargs))

	def reset_index(self, level=None, drop=False, inplace=False, col_level=0, col_fill=''):
		return self._check_and_change(super().reset_index(level=level, drop=drop, inplace=inplace, col_level=col_level, col_fill=col_fill))

	def set_index(self, keys, drop=True, append=False, inplace=False, verify_integrity=False):
		return self._check_and_change(super().set_index(keys=keys, drop=drop, append=append, inplace=inplace, verify_integrity=verify_integrity))

	def dropna(self, axis=0, how='any', thresh=None, subset=None, inplace=False):
		return self._check_and_change(super().dropna(axis=axis, how=how, thresh=thresh, subset=subset, inplace=inplace))

	def fillna(self, value=None, method=None, axis=None, inplace=False, limit=None, downcast=None, **kwargs):
		return self._check_and_change(super().fillna(value=value, method=method, axis=axis, inplace=inplace, limit=limit, downcast=downcast, **kwargs))

	def ffill(self, axis=None, inplace=False, limit=None, downcast=None):
		return self._check_and_change(super().ffill(axis=axis, inplace=inplace, limit=limit, downcast=downcast))

	def bfill(self, axis=None, inplace=False, limit=None, downcast=None):
		return self._check_and_change(super().bfill(axis=axis, inplace=inplace, limit=limit, downcast=downcast))
	...
	def replace(self, to_replace=None, value=None, inplace=False, limit=None, regex=False, method='pad'):
		return self._check_and_change(super().replace(to_replace=to_replace, value=value, inplace=inplace, limit=limit, regex=regex, method=method))
	...
	def drop(self, labels=None, axis=0, index=None, columns=None, level=None, inplace=False, errors='raise'):
		return self._check_and_change(super().drop(labels=labels, axis=axis, index=index, columns=columns, level=level, inplace=inplace, errors=errors))
...
class ConvertibleFrame(BaseFrame, metaclass=abcd.ABCMeta):
	"""
...
class OrganizingFrame(ConvertibleFrame):
	...
	def convert(cls, df: pd.DataFrame, require_full: bool = _sentinel):
		...
		if require_full is _sentinel:
			...
			warn("Passing require_full to OrganizingFrame.convert is deprecated.", DeprecationWarning)
		...
		if require_full:
			for c in set(cls.required_index_names()):
				if c not in set(df.index.names):
					raise MissingColumnError("Missing index name {}".format(c), key=c)
			for c in set(cls.required_columns()):
				if c not in set(df.columns):
					raise MissingColumnError("Missing column {}".format(c), key=c)
		if not cls.more_columns_allowed():
			for c in df.columns:
				if c not in cls.required_columns() and c not in cls.reserved_columns():
					raise UnexpectedColumnError("Unexpected column {}".format(c), key=c)
		if not cls.more_index_names_allowed() and list(df.index.names)!=['None']:
			for c in df.index.names:
				if c is None or c not in cls.required_index_names() and c not in cls.reserved_index_names():
					raise UnexpectedColumnError("Unexpected column {}".format(c), key=c)


__all__ = ['PrettyFrame', 'BaseFrame', 'SimpleFrame', 'FinalFrame', 'OrganizingFrame', 'ConvertibleFrame']
