...
class OrganizingFrame(ConvertibleFrame):
    ...
    def convert(cls, df: pd.DataFrame, require_full: bool = _sentinel):
        ...
        if require_full is _sentinel:
            ...
            warn(
                "Passing require_full to OrganizingFrame.convert is deprecated.", DeprecationWarning
            )
    ...
    def read_csv(cls, path: PathLike, *args, **kwargs):
        if "index_col" in kwargs:
            warn(
                "index_col={} in OrganizingFrame.read_csv is ignored".format(kwargs["index_col"]),
                IgnoringRequestWarning,
            )
